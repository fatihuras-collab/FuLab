...
foreach(['FULAB_ORIGIN'=>'origin','RESEND_API_KEY'=>'resend_key','FULAB_MAIL_FROM'=>'mail_from','OPENAI_API_KEY'=>'openai_key','OPENAI_MODEL'=>'openai_model'] as $env=>$key){$value=getenv($env);if($value!==false)$cfg[$key]=$value;}
foreach(['FULAB_PRIVACY_READY'=>'privacy_ready','FULAB_AI_ENABLED'=>'ai_enabled'] as $env=>$key){$value=getenv($env);if($value!==false)$cfg[$key]=filter_var($value,FILTER_VALIDATE_BOOLEAN);}
...
$resendReady = $ready && !empty($cfg['resend_key']) && filter_var($cfg['mail_from']??'',FILTER_VALIDATE_EMAIL) && function_exists('curl_init');
$mailReady = $mailReady || $resendReady;
...
session_set_cookie_params(['httponly'=>true,'secure'=>(str_starts_with($origin,'https://') || (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS']!=='off')),'samesite'=>'Strict','path'=>'/']);
session_start();
...
if($action==='lead'){
    ...
    if($resendReady){
        $payload=['from'=>'FuLab Web <'.$cfg['mail_from'].'>','to'=>['fatih@fulabtr.net'],'subject'=>'FuLab — Yeni proje talebi','text'=>"Müşterinin onayladığı talep özeti\n\n".$summary."\n\nAd / şirket: ".$name."\nİletişim: ".$contact];
        if(filter_var($contact,FILTER_VALIDATE_EMAIL))$payload['reply_to']=$contact;
        $curl=curl_init('https://api.resend.com/emails');
        curl_setopt_array($curl,[CURLOPT_POST=>true,CURLOPT_RETURNTRANSFER=>true,CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>25,CURLOPT_HTTPHEADER=>['Authorization: Bearer '.$cfg['resend_key'],'Content-Type: application/json','Idempotency-Key: fulab-'.hash('sha256',session_id().$hash)],CURLOPT_POSTFIELDS=>json_encode($payload,JSON_UNESCAPED_UNICODE)]);
        $result=curl_exec($curl);$status=curl_getinfo($curl,CURLINFO_HTTP_CODE);curl_close($curl);$result=json_decode($result?:'{}',true);
        if($status>=200 && $status<300 && !empty($result['id'])){$_SESSION['sent_hash']=$hash;reply(200,['ok'=>true,'message'=>'Talebiniz FuLab’a iletilmek üzere e-posta hizmeti tarafından kabul edildi.']);}
        reply(502,['ok'=>false,'message'=>'E-posta gönderilemedi. Bilgileriniz korunuyor; tekrar deneyebilir veya WhatsApp ile paylaşabilirsiniz.']);
    }
